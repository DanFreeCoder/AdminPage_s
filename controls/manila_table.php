...
$sql1 = "SELECT * FROM locals WHERE trunkline != 1"; //for counter display
$get_all_rows = $code->manila_locals($sql1);
...
if (isset($_POST['search']['value'])) {
    ...
    $sql .= " WHERE id LIKE '%" . $searh_value . "%' AND trunkline != 1";
    $sql .= " OR local_no LIKE '%" . $searh_value . "%' AND trunkline != 1";
    $sql .= " OR name LIKE '%" . $searh_value . "%' AND trunkline != 1";
    $sql .= " OR department LIKE '%" . $searh_value . "%' AND trunkline != 1";
    $sql .= " OR status LIKE '%" . $searh_value . "%' AND trunkline != 1";
} else {
    $sql .= " WHERE id LIKE '%%' AND trunkline != 1";
}
...
$run_query = $code->manila_locals($sql);
...
while ($row = $run_query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $id = '<input type="checkbox" name="form_manila" class="checklist" value="' . $row['id'] . '">';
